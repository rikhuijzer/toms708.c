...
/* Based on C translation of ACM TOMS 708
   ...
   More specifically,  Brown & Levy (1994) "Certification of Algorithm 708"
   write
   "
    ...
    (1) an average of 13.71 significant digits are obtained in cases in which a
   = b and (2) the number increases 0.14 significant digits for each unit change
   in log10(a/b).
   "
*/
 ...
 *MM (w/ Debug, w/o Optimization):
 (cd `R-devel-pbeta-dbg RHOME`/src/nmath ; gcc -I. -I../../src/include
 -I../../../R/src/include  -DHAVE_CONFIG_H -fopenmp -g -pedantic -Wall
 --std=gnu99 -DDEBUG_q -DDEBUG_bratio -Wcast-align -Wclobbered  -c
...
double i1mach(int i) {
  ...
  case 6:
    return sizeof(int) / sizeof(char); // NOLINT
  case 7:
    return 2;
  case 8:
...
double r_d__0(bool log_p) { return log_p ? ML_NEGINF : 0.0; }

double r_d__1(bool log_p) { return log_p ? 0.0 : 1.0; }

double r_d_exp(bool log_p, double x) { return log_p ? x : exp(x); }
...
void bratio(double a, double b, double x, double y, double *w, double *w1,
  ...
  *ierr = 0;
  ...
  eps = max(eps, 1e-15);
  bool a_lt_b = (a < b);
  if (/* max(a,b) */ (a_lt_b ? b : a) <
      eps * .001) { /* procedure for a and b < 0.001 * eps */
...
static double bup(double a, double b, double x, double y, int n, double eps,
  ...
  /* L10: */
  ret_val = give_log ? brcmp1(mu, a, b, x, y, true) - log(a)
                     : brcmp1(mu, a, b, x, y, false) / a;
  if (n == 1 || (give_log && ret_val == ML_NEGINF) ||
      (!give_log && ret_val == 0.))
    return ret_val;
...
static double bfrac(double a, double b, double x, double y, double lambda,
  ...
  double c, e, n, p, r, s, t, w, c0, c1, r0, an, bn, yp1, anp1, bnp1, beta,
      alpha, brc;

  if (!isfinite(lambda))
    return NAN; // TODO: can return 0 or 1 (?)
  R_ifDEBUG_printf(
      " bfrac(a=%g, b=%g, x=%g, y=%g, lambda=%g, eps=%g, log_p=%d):", a, b, x,
      y, lambda, eps, log_p);
  ...
  do {
    ...
    R_ifDEBUG_printf(" n=%5.0f, a_{n,n+1}= (%12g,%12g),  b_{n,n+1} = "
                     "(%12g,%12g) => r0,r = (%14g,%14g)\n",
  ...
  if (n >= 10000 && fabs(r - r0) > eps * r)
    printf(" bfrac(a=%g, b=%g, x=%g, y=%g, lambda=%g) did *not* converge (in "
           "10000 steps)\n",
           a, b, x, y, lambda);
...
static double brcomp(double a, double b, double x, double y, int log_p) {
  ...
  if (a0 < 8.) {
    ...
    return (log_p ? log(a0) + z + log1p(gam1(b0)) - log(t)
    ...
    return (log_p ? -M_LN_SQRT_2PI + .5 * log(b * x0) + z - bcorr(a, b)
...
static double brcmp1(int mu, double a, double b, double x, double y,
  ...
  if (a0 < 8.) {
    ...
    if (b0 >= 8.) {
      ...
      return give_log ? log(a0) + esum(mu, z - u, true)
      ...
      // L50:
      c = give_log ? log1p(gam1(a)) + log1p(gam1(b)) - log(z)
                   : (gam1(a) + 1.) * (gam1(b) + 1.) / z;
      R_ifDEBUG_printf(" brcmp1(mu,a,b,*): a0 < 1, b0 <= 1;  c=%.15g\n", c);
      return give_log ? ans + log(a0) + c - log1p(a0 / b0)
                      : ans * (a0 * c) / (a0 / b0 + 1.);
    }
    ...
    // L72:
    return give_log ? log(a0) + esum(mu, z, true) + log1p(gam1(b0)) -
                          log(t) // TODO? log(t) = log1p(..)
    ...
    return give_log ? log(const__) + (log(b) + lx0) / 2. + z - bcorr(a, b)
                    : const__ * sqrt(b * x0) * z * exp(-bcorr(a, b));
  }

} /* brcmp1 */

static void bgrat(double a, double b, double x, double y, double *w, double eps,
                  int *ierr, bool log_w) {
  /* -----------------------------------------------------------------------
...
#define n_terms_bgrat 30
  ...
  double l = // := *w/u .. but with care: such that it also works when u
             // underflows to 0:
      log_w ? ((*w == ML_NEGINF) ? 0. : exp(*w - log_u))
            : ((*w == 0.) ? 0. : exp(log(*w) - log_u));

  R_ifDEBUG_printf(" bgrat(a=%g, b=%g, x=%g, *)\n -> u=%g, l='w/u'=%g, ", a, b,
                   x, u, l);
  double q_r =
             grat_r(b, z, log_r, eps), // = q/r of former grat1(b,z, r, &p, &q)
      v = 0.25 / (nu * nu), t2 = lnx * 0.25 * lnx, j = q_r, sum = j, t = 1.,
         cn = 1., n2 = 0.;
...
static double grat_r(double a, double x, double log_r, double eps) {
  ...
  if (a * x == 0.) { /* L130: */
    ...
    } else { // 2013-02-27: improvement for "large" x: direct computation of
             // q/r:
      double sx = sqrt(x), q_r = erfc1(1, sx) / sx * M_SQRT_PI;
      R_ifDEBUG_printf(
    ...
    double an = 3., c = x, sum = x / (a + 3.), tol = eps * 0.1 / (a + 1.), t;
    do {
      an += 1.;
      c *= -(x / an);
      t = c / (a + an);
    ...
    double j = a * x * ((sum / 6. - 0.5 / (a + 2.)) * x + 1. / (a + 1.)),
           z = a * log(x), h = gam1(a), g = h + 1.;

    if ((x >= 0.25 && (a < x / 2.59)) || (z > -0.13394)) {
      // L40:
      double l = rexpm1(z), q = ((l + 0.5 + 0.5) * j - l) * g - h;
      if (q <= 0.) {
    ...
    double a2n_1 = 1., a2n = 1., b2n_1 = x, b2n = x + (1. - a), c = 1., am0,
           an0;
...
#define num_IT 20
  ...
  double z0 = sqrt(f), z = z0 / e1 * 0.5, z2 = f + f, h, r0, r1, w0;
  ...
  c[0] = a0[0] * -0.5;
  d[0] = -c[0];
  double j0 = 0.5 / e0 * erfc1(1, z0), j1 = e1, sum = j0 + d[0] * w0 * j1;

  double s = 1., h2 = h * h, hn = 1., w = w0, znm1 = z, zn = z2;
...
static double exparg(int l) {
   ...
   *     exp(W) can be computed. With 0.99999 fuzz  ==> exparg(0) =   709.7756
   nowadays
...
static double alnrel(double a) {
  ...
  if (fabs(a) > 0.375)
    return log(1. + a);
  // else : |a| <= 0.375
  static double p1 = -1.29418923021993, p2 = .405303492862024,
                p3 = -.0178874546012214, q1 = -1.62752256355323,
                q2 = .747811014037616, q3 = -.0845104217945565;
  double t = a / (a + 2.), t2 = t * t,
...
static double erf__(double x) {
  ...
  static double q[8] = {1.,
                        ...
                        790.950925327898,
                        300.459260956983};
  static double r[5] = {2.10144126479064, 26.2370141675169, 21.3688200555087,
                        4.6580782871847, .282094791773523};
  static double s[4] = {94.153775055546, 187.11481179959, 99.0191814623914,
                        18.0124575948747};
...
static double gam1(double a) {
  ...
  if (t < 0.) { /* L30: */
    static double r[9] = {-.422784335098468,   -.771330383816272,
                          ...
                          9.30357293360349e-4, -.0118290993445146,
                          .00223047661158249,  2.66505979058923e-4,
                          -1.32674909766242e-4},
                  s1 = .273076135303957, s2 = .0559398236957378;
...
static double psi(double x) {
  ...
  static double q2[4] = {32.2703493791143, 89.2920700481861, 54.6117738103215,
                         7.77788548522962};
  /* --------------------------------------------------------------------- */
...
static double betaln(double a0, double b0) {
  ...
  double a = min(a0, b0), b = max(a0, b0);
  ...
  if (a < 8.) {
    ...
    double w = bcorr(a, b), h = a / b, u = -(a - 0.5) * log(h / (h + 1.)),
           v = b * alnrel(h);
    if (u > v)
...
static double bcorr(double a0, double b0) {
  ...
  r1 = 1. / b;
  t = r1 * r1;
  w = ((((c5 * s11 * t + c4 * s9) * t + c3 * s7) * t + c2 * s5) * t + c1 * s3) *
          t +
      c0;
  w *= c / b;
...
static double gamln(double a) {
  ...
  if (a <= 0.8)
    return gamln1(a) -
           log(a); /* ln(G(a+1)) - ln(a) == ln(G(a+1)/a) = ln(G(a)) */
  else if (a <= 2.25)
